...
select
	...
	(case when smr.cost <> 0 then  (case when dsc.percent_value is not null then (smr.cost *  (dsc.percent_value::int / 10)) /10 else smr.cost * 1 end) end) as base_price,
	pmt_prt.amount as ykassa,
    extract(year from sme.started_at::timestamp at time zone 'UTC') as cd_year,
    extract(month from sme.started_at::timestamp at time zone 'UTC')  as cd_month,
    extract(day from sme.started_at::timestamp at time zone 'UTC')  as cd_day,
    smr.started_at::timestamp at time zone 'UTC' as cd_data_smr_start,
    sme.performed_at::timestamp at time zone 'UTC' as cd_data_smr_closed,
    sme.created_at::timestamp at time zone 'UTC' as cd_data_smr_creat,
